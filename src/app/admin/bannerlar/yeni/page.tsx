...
export default function YeniBannerPage() {
  ...
  const handleSave = async () => {
    if (!title.trim() || !imageUrl.trim()) { setError("Başlık ve görsel URL zorunludur."); return; }
    ...
    try {
      const res  = await fetch("/api/admin/bannerlar", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      const json = await res.json() as { success: boolean; error?: string };
      if (!json.success) { setError(json.error ?? "Banner oluşturulamadı."); return; }
  ...
  return (
    <div className="flex flex-col h-full">
      <AdminHeader
        title="Yeni Banner"
        actions={
          <Link href="/admin/bannerlar" className="flex items-center gap-1.5 text-sm text-ink-muted hover:text-ink transition-colors">
            <ChevronLeft size={16} /> Bannerlar
          </Link>
        }
      />
      ...
      <div className="p-6 flex-1 overflow-auto">
        <div className="space-y-5 max-w-xl">
          <Field label="Başlık *">
            <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="Yaz Kampanyası" className={inputCls} />
          </Field>
          ...
          <Field label="Bağlantı URL">
            <input type="url" value={linkUrl} onChange={e => setLinkUrl(e.target.value)} placeholder="https://… (isteğe bağlı)" className={inputCls} />
          </Field>
